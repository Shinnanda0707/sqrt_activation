...
parser.add_argument("-s", "--slope", dest="slope", type=float, default=1.0)
...
model = SqrtCNN(slope=args.slope)
model.cuda()
model.load_state_dict(torch.load("/home/pie/Desktop/Python/AI/SQRT_activation/CNN_init.pth"))
optimizer = optim.SGD(model.parameters(), lr=args.learning_rate)
loss = nn.CrossEntropyLoss()

tb = SummaryWriter(os.path.join("/home/pie/Desktop/Python/AI/SQRT_activation/runs", f"SQRT_{args.learning_rate}_{args.slope}"))
print(f"{'=' * 35}[Start training SQRT with lr={args.learning_rate}, slope={args.slope}]{'=' * 35}")
st = time.time()
...
for i in range(args.epoch):
    ...
    torch.save(model.state_dict(), os.path.join(f"/home/pie/Desktop/Python/AI/SQRT_activation/SQRT/{args.learning_rate}/{args.slope}", f"{i + 1}.pth"))
